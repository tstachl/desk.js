...
Client.prototype.onResponse = function(callback, options, err, response, body) {
  if (err) {
    ...
    } else if (err.code === 'ECONNRESET' || err.message === 'socket hang up') {
      if (this.retry && options.retryCount <= this.maxRetry) return this.doRetry(options, callback);
  ...
  if (response === null || !response.hasOwnProperty('statusCode') || !response.statusCode) {
    callback(); // clears the item from the queue
    return options.callback(new Error('Failed to get response object'));
  }
  ...
  if (response.statusCode === 429) {
    ...
    callback(); // clears the item from the queue
    if (response.statusCode > 299 && body.hasOwnProperty('message')) {
